...
protected:
	...
	static const int N_WORDS = ((K + 7) / 8 + sizeof(mp_limb_t) - 1) / sizeof(mp_limb_t);
...
public:
	...
	Z2(long x) : Z2(mp_limb_t(x))
	{
		if (K > 64 and x < 0)
			memset(&a[1], -1, N_BYTES - 8);
	}
...
public:
	...
	template <class T>
	SignedZ2(const T &other) : Z2<K>(other)
	{
	}

	template <int L>
	SignedZ2(const fixint<L> &other) : Z2<K>(other)
	{
		extend(other);
	}
