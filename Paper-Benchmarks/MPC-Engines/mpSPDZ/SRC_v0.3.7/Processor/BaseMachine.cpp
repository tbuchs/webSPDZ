...
void BaseMachine::print_timers()
{
  ...
  for (auto it = timer.begin(); it != timer.end(); it++) {
    cerr << "Time" << it->first << " = " << it->second.elapsed() << " seconds ("
     ...
     /*
      <b3m4>
      {"timer":1,
       "time":{"seconds":1.027240,"ms":1027.2402},
      }
      </b3m4>
      */
    double time_secs = it->second.elapsed();
  ...
  //printf(BENCH_TEXT_BOLD BENCH_COLOR_GREEN BENCH_MAGIC_START
    cerr  
      ...
      << "{\"timer\":" << timer_id << "," << endl
      << " \"time\":{\"sec\":"<<time_secs << ",\"ms\":"<<(time_secs*1000) << "}" << endl
      << "}" << endl
      << BENCH_MAGIC_END << BENCH_ATTR_RESET << endl;
  }
}
...
void BaseMachine::print_comm(Player& P, const NamedCommStats& comm_stats)
{
  ...
  cerr
  << BENCH_TEXT_BOLD << BENCH_COLOR_BLUE << BENCH_MAGIC_START 
  << "{\"player\":" << P.my_num() << "," << endl
  << "  \"netdata\":{" << endl
  << "    \"sent\":{\"bytes\":"<<comm_stats.sent << ",\"MB\":"<<comm_stats.sent/1e6 << "}" << endl 
  //<< "    \"received\":{\"bytes":26683968,\"MB":26.68}" << endl
  << "  }," << endl
  << "  \"roundsdata\":{" << endl
  //<< "    \"broadcast\":7," << endl
  //<< "    \"p-to-p\":2136" << endl
  ...
  {"player":0,
    "thread":2,
    "netdata":{
      "sent":{"bytes":26684013,"MB":26.68},
      "received":{"bytes":26683968,"MB":26.68}
    },
    "roundsdata":{
      "broadcast":7,
      "p-to-p":2136
